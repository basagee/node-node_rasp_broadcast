            ...
            if (resCode === '0000') {
                ...
                if (payloadJson.SERVICE_TYPE === '00' || payloadJson.SERVICE_TYPE === '01') {
                    ...
                    if (remote_play)*/ {
                        ...
                        jsutils.showModalWindow(mediaBrowserPath)
                    } /*else {
                        var arrStr = payloadJson.MESSAGE.split('?')
                        var httpConfig = config.getServerConfig();

                        var mediaBrowserPath = 'file://' + httpConfig.staticDirectory + '/www/media/';
                        if (payloadJson.SERVICE_TYPE === '00') {
                            mediaBrowserPath += 'realtime_media.html'
                        } else {
                            mediaBrowserPath += 'normal_media.html'
                        }

                        if (arrStr.length === 2) {
                            mediaBrowserPath += '?' + arrStr[1];
                            mediaBrowserPath += '&UUID=' + config.getDeviceId() + '&APPID=' +
                                                    config.getApplicationPackageName()
                        } else {
                            mediaBrowserPath += '?UUID=' + config.getDeviceId() + '&APPID=' +
