...
function PushClient() {
    ...
    // media app(pjsua) telnet connection
    this.mediaAppConnection = new telnet();
    closedMediaAppConnectionBySystem = false;
    this.mediaAppConnection.on('ready', this.onMediaAppConnectionReady);
    this.mediaAppConnection.on('connect', this.onMediaAppConnectionConnect);
    this.mediaAppConnection.on('data', this.onMediaAppConnectionData);
    this.mediaAppConnection.on('error', this.onMediaAppConnectionError);
    this.mediaAppConnection.on('timeout', this.onMediaAppConnectionTimeout);
    this.mediaAppConnection.on('close', this.onMediaAppConnectionClose);
}
...
PushClient.prototype.start = function(options) {
    ...
    if (!webkit_play) {
        ...
        closedMediaAppConnectionBySystem = false;
        isMediaApplicationStarted = false;

        this.execMediaApplication()
        setTimeout(function() {
            PushClient.getInstance().openMediaAppConnection();
        }, 2000);
...
// media app telnet
var closedMediaAppConnectionBySystem = false;
PushClient.prototype.openMediaAppConnection = function() {
    console.log('openMediaAppConnection()')
    var self = PushClient.getInstance();
    try {
        self.mediaAppConnection.connect(mediaAppTelnetParams);
    } catch (e) {
        console.error(e)
    }
}

PushClient.prototype.closeMediaAppConnectionBySystem = function() {
    console.log('closeMediaAppConnectionBySystem()')
    closedMediaAppConnectionBySystem = true;
    var self = PushClient.getInstance();
    try {
        self.mediaAppConnection.end();
    } catch (e) {
        console.error(e)
    }
}

PushClient.prototype.sendBroadcastCallToMediaApp = function(brcnum) {
    console.log('openMediaAppConnection()')
    var self = PushClient.getInstance();
    self.mediaAppConnection.send("call new sip:brc{0}@nbplus.co.kr".format(brcnum), function(err, response) {
        console.log(err)
        console.log(response)
    });
}

PushClient.prototype.onMediaAppConnectionReady = function(prompt) {
    var self = PushClient.getInstance();
    console.log('onMediaAppConnectionReady()')
}

PushClient.prototype.onMediaAppConnectionConnect = function(prompt) {
    var self = PushClient.getInstance();
    console.log('onMediaAppConnectionConnect()')
}

PushClient.prototype.onMediaAppConnectionData = function(buffer) {
    console.error('======================== onMediaAppConnectionData()')
    console.error(buffer)
}

PushClient.prototype.onMediaAppConnectionTimeout = function() {
    console.log('onMediaAppConnectionTimeout()')
    var self = PushClient.getInstance();
    try {
        self.mediaAppConnection.destroy()
    } catch (e) {
        console.error(e)
    }
}

PushClient.prototype.onMediaAppConnectionError = function(error) {
    console.error('======================== onMediaAppConnectionError()')
    console.error(error)
}

PushClient.prototype.onMediaAppConnectionClose = function() {
    console.log('onMediaAppConnectionClose()')
    if (!closedMediaAppConnectionBySystem) {
        setTimeout(function () {
            PushClient.getInstance().openMediaAppConnection()
        }, 1500);
    }
