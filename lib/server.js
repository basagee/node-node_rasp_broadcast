...
function Server() {
    ...
    server.post('/api/register', function (req, res, next) {
     ...
     server.get('/api/gwinfo', function (req, res, next) {
         ...
         try {
             if (!jsutils.isNullObject(ble)) {
                 response.data.iotcontroller.ble = ble.isEnabled();
             }
             if (!jsutils.isNullObject(ozw)) {
...
function registerGateway(userid, passwd, res, next) {
    ...
    var httpRequest = new XMLHttpRequest();
    httpRequest._userAgent = "Mozilla/5.0 (Linux; Android 6.0; Lenovo TB3-730F Build/MRA58K; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/55.0.2883.91 Safari/537.36";
    ...
    */
    ...
    if (isJson) {
        ...
        */
        ...
        params += '&master_domain=' + encodeURIComponent(user.server.api);
    ...
    if (isJson) {
        httpRequest.responseType = 'json';
        httpRequest.setRequestHeader("Content-Type", "application/json");
        //httpRequest.setRequestHeader('cookie', 'JSESSIONID=DBF82F2FB26C2DE3C866B61B9D722B57')
        //httpRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    } else {
...
Server.prototype.start = function() {
     ...
     */
     ...
     if (isRegisteredGateway) {
         ...
         logger.debug('-- found registered user = ' + userId + ', deviceid = ' + config.getDeviceId());
...
Server.prototype.stop = function() {
    console.log('Server.prototype.stop() called.....');
    if (!jsutils.isNullObject(mdnsService)) {
        mdnsService.stop();
    }
