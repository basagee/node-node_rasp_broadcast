...
var configuration = function configuration(){
    ...
    this.init = function() {
        try {
            _config = JSON.parse(fs.readFileSync(appDir + '/config.data' + '/config.json', "utf8"));
        } catch (e) {
            if (e.code === 'ENOENT') {
                fs.appendFile(appDir + '/config.data' + '/config.json', '', function(err) {
                    //logger.error(err);
                });
            }
        }
        ...
        if (utils.isNullObject(_httpConfig)) {
            ...
            _httpConfig.address = 'http://localhost';
            _httpConfig.port = 8090;
            ...
            if (utils.isNullObject(_httpConfig.address)) {
                _httpConfig.address = 'http://localhost';
            }
    ...
    this.isLocalElectron = function() {
        return _config.isLocalElectron;
    }
