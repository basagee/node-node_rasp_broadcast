...
var User = function User(){
    ...
    this.init = function() {
        try {
            _userList = JSON.parse(fs.readFileSync(__dirname + '/user.json', "utf8"));
        } catch (e) {
            if (e.code === 'ENOENT') {
                fs.appendFile(__dirname + '/user.json', '', function(err) {
                    //logger.error(err);
                });
            }
        }
    ...
    this.update = function(userinfo) {
        ...
        fs.writeFileSync(__dirname + '/user.json', JSON.stringify(_userList), "utf8");
    };
